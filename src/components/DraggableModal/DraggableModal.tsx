...
export type DraggableModalProps = DraggableModalType & {
...
export const DraggableModal = ({
  ...
  return (
    <motion.div
      ...
      className="fixed h-[min(80vh,800px)] w-[clamp(480px,35vw,600px)] cursor-grab bg-primary shadow-[rgba(0,0,0,0.35)_0px_5px_15px]"
    >
      <button
      ...
      <div className="flex h-full flex-col gap-8 overflow-auto p-10">
        <h4 className="bg-transparent text-center font-menu text-[2.2rem] leading-[2.2rem] tracking-widest [-webkit-text-stroke:1px_#fff] [text-shadow:6px_6px_4px_rgba(0,0,0,0.3)]">
          {heading}
        </h4>
        <div className="h-[calc(100%+2.5rem)] font-modal text-base">
          {children}
        </div>
      </div>
    </motion.div>
